...
## Tutorials (standalone notebooks)

Two generated notebooks under `tutorials/` demonstrate the pipeline **without the DIMER workers**: they are
**standalone** under DIMER Notebook Specification 1.1 §3.6 — generated by `tools/build_notebook.py` from
`tools/notebook_template.py` / `tools/notebook_template_artifact_inference.py`, they carry
`src/tabpfn_regressor_pipeline/pipeline.py` verbatim, the 4-file snapshot manifest of the pinned
`Prior-Labs/tabpfn_3` checkpoint (`tabpfn-v3-regressor-v3_default.ckpt` at immutable revision
`24a16a89d245878b846555110985634aa2e656d7`, see `docs/WEIGHTS.md`) and the `pyproject.toml` runtime pins inline, so the
exported `.ipynb` needs no clone, no package install, no token and no access to this repository.
...
weights are non-commercial (`tabpfn-3-license-v1.0`), so the standalone notebooks stop at in-context inference and say
so; the DIMER workers remain the fine-tuning path of this pipeline. The notebooks pin the **v3** generation only.

Public package API (offline unit tests in `tests/test_role_helpers.py`; `tabpfn` / `torch` are imported lazily):

```python
from tabpfn_regressor_pipeline import TabPFNRegressorPipeline, build_synthetic_dataset, read_dataset_zip, validate_inputs, evaluation_report
pipe = TabPFNRegressorPipeline.from_pretrained(weights_dir="weights/tabpfn-3-regressor")   # digest-verified checkpoint
frames = read_dataset_zip(build_synthetic_dataset("synthetic.zip"))
manifest = validate_inputs(frames["train.csv"], "target", val=frames["val.csv"])  # raises InputRejected
pipe.fit(X_train, y_train, target_column="target")           # in-context: no gradient update
metrics = pipe.evaluate(X_val, y_val)
pipe.save_artifact("artifact")                               # model.tabpfn_fit + model.ckpt + artifact_manifest.json
fresh = TabPFNRegressorPipeline.from_artifact("artifact")            # validates the bundle, then reloads
```

Static gates (also run by CI): `ruff check .`, `pytest -m "not integration"`, `python tools/validate_release_assets.py`,
`python tools/build_notebook.py --check` (and `--check --template tools/notebook_template_artifact_inference.py`).

## Release status

**Candidate — tutorial source complete, awaiting clean-runtime execution** (`STATUS.md`). The standalone notebooks pass
static validation and the generator parity checks; the clean-runtime run of the exact candidate revision is recorded —
and gates promotion — in `docs/release-verification.md`.
